...
public class movieQualityController 
{
	...
	ResponseEntity<MovieQualityDto> addQuality(@PathVariable("movieId") Integer movieId, @RequestBody MovieQualityDto qualityDto){
		MovieQualityDto mq = mqs.addMovieQuality(movieId, qualityDto);
		return ResponseEntity.ok(mq);
	}
	
	@PutMapping("quality/{qualityId}")
	ResponseEntity<MovieQualityDto> updateQuality(@PathVariable("qualityId") Integer qualityId ,@RequestBody MovieQualityDto qualityDto){
		MovieQualityDto mq = mqs.updateQuality(qualityId, qualityDto);
		return ResponseEntity.ok(mq);
	}
	
	@DeleteMapping("quality/{qualityId}")
	ResponseEntity<String> deleteQuality(@PathVariable("qualityId") Integer qualityId){
		mqs.deleteQuality(qualityId);
		return ResponseEntity.ok("Quality deleted");
	}
	
	@GetMapping("{movieId}/quality")
	ResponseEntity<List<MovieQualityDto>> getAllQuality(@PathVariable("movieId") Integer movieId){
		List<MovieQualityDto> mq = mqs.getAllQuality(movieId);
		return ResponseEntity.ok(mq);
	}
	
	@GetMapping("quality/{qualityId}")
	ResponseEntity<MovieQualityDto> getQualityById(@PathVariable("qualityId") Integer qualityId){
